        ...
        <div style="display: grid; grid-template-columns: 2.2fr 1fr; gap: 2rem; align-items: start;">
          ...
          <div>
            ...
            <div class="watermarked-image-container" style="position: relative; margin-bottom: 1.75rem; border-radius: 16px; overflow: hidden; border: 3px solid var(--forest-dk); box-shadow: var(--shadow-lg);">
              <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 0.5rem; background: var(--forest-dk);">
                <img src="${prop.images[0]}" alt="${tText(prop.title)}" style="width: 100%; height: 380px; object-fit: cover;" />
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                  <img src="${prop.images[1] || prop.images[0]}" alt="Property interior 1" style="width: 100%; height: 186px; object-fit: cover;" />
                  <img src="${prop.images[2] || prop.images[0]}" alt="Property interior 2" style="width: 100%; height: 186px; object-fit: cover;" />
                </div>
              </div>

              <!-- Interactive 3D Walkthrough Toggle Overlay -->
              <button type="button" id="toggle-3d-walkthrough-btn" class="btn btn-dark" style="position: absolute; bottom: 20px; left: 20px; background: rgba(19, 29, 12, 0.95); color: var(--marigold); border: 2px solid var(--marigold); font-size: 0.9rem; font-weight: 800; padding: 12px 22px; border-radius: 30px; box-shadow: 0 8px 25px rgba(0,0,0,0.6); display: flex; align-items: center; gap: 8px; cursor: pointer;">
                ${renderIcon('sparkles', 18, 'var(--marigold)')} 🕶️ ${isVirtualTourOpen ? 'Hide 3D Tour' : 'Launch 360° Virtual Walkthrough'}
              </button>
            </div>

            ${isVirtualTourOpen ? `
              <!-- Embedded 3D Virtual Tour Viewer -->
              <div style="background: #0F172A; color: white; border-radius: 16px; padding: 1.5rem; margin-bottom: 1.75rem; border: 3px solid var(--marigold); box-shadow: var(--shadow-lg);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                  <h4 style="color: var(--marigold); margin: 0; font-size: 1.1rem; display: flex; align-items: center; gap: 8px;">
                ...
                <div style="width: 100%; height: 400px; border-radius: 12px; overflow: hidden; background: #000; position: relative;">
            ...
            <div style="background: var(--paper); border-radius: 16px; padding: 1.75rem; border: 2px solid var(--border-dk); box-shadow: var(--shadow-md); margin-bottom: 1.75rem;">
              <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 1rem; margin-bottom: 1rem;">
                <div>
                  ...
                  <h1 style="font-size: 1.8rem; color: var(--forest-dk); line-height: 1.25; margin-bottom: 0.5rem; font-weight: 800;">
                    ${tText(prop.title)}
                  </h1>
                  <div style="color: var(--forest); font-size: 1rem; font-weight: 600; display: flex; align-items: center; gap: 6px;">
                ...
                <div style="text-align: right;">
                  <div style="font-family: var(--font-mono); font-size: 2.4rem; font-weight: 800; color: var(--rani-dk);">
              ...
              <!-- Key Specs Grid -->
              <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; background: var(--cream); padding: 1.25rem; border-radius: 12px; text-align: center; border: 2px solid var(--border-dk);">
                <div>
            ...
            <div style="background: var(--paper); border-radius: 16px; padding: 1.75rem; border: 2px solid var(--border-dk); box-shadow: var(--shadow-md); margin-bottom: 1.75rem;">
              <h3 style="margin-bottom: 1rem; color: var(--forest-dk); font-size: 1.25rem; font-weight: 800; border-bottom: 2px solid var(--cream); padding-bottom: 0.5rem;">
                ${t('property_overview', 'Property Overview & Detailed Specs')}
              </h3>
              <p style="color: var(--ink); font-size: 1rem; line-height: 1.8; margin-bottom: 1.5rem;">${tText(prop.description)}</p>

              <h4 style="margin-bottom: 1rem; color: var(--forest-dk); font-size: 1.1rem; font-weight: 800;">
                Key Features & Premium Amenities
              </h4>
              <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
                ${prop.features.map(f => `
                  <div style="display: flex; align-items: center; gap: 8px; font-size: 0.92rem; font-weight: 700; color: var(--forest-dk); background: var(--cream); padding: 10px 14px; border-radius: 10px; border: 1px solid var(--border-dk);">
                    ${renderIcon('check-circle', 18, 'var(--rani)')} ${tText(f)}
                  </div>
                `).join('')}
              </div>
            </div>
            ...
            <div style="background: var(--paper); border-radius: 16px; padding: 1.75rem; border: 2px solid var(--forest-dk); box-shadow: var(--shadow-md); margin-bottom: 1.75rem;">
              <h3 style="margin-bottom: 0.5rem; color: var(--forest-dk); font-size: 1.25rem; font-weight: 800; display: flex; align-items: center; gap: 8px;">
                ${renderIcon('calendar', 20, 'var(--rani)')} Book a Physical Site Visit & Guided Tour
              </h3>
              <p style="color: var(--forest); opacity: 0.85; font-size: 0.88rem; margin-bottom: 1.25rem;">
          ...
          <div style="display: flex; flex-direction: column; gap: 1.5rem;">
            ...
            <div style="background: var(--forest-dk); color: white; border-radius: 16px; padding: 1.75rem; border: 3px solid var(--forest); box-shadow: var(--shadow-lg);">
              <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1.25rem;">
                <img src="${prop.agency.avatar}" alt="${prop.agency.agentName}" style="width: 4.5rem; height: 4.5rem; border-radius: 50%; border: 3px solid var(--marigold); object-fit: cover;" />
                <div>
                  <h4 style="color: white; font-size: 1.2rem; margin: 0; font-weight: 800;">${prop.agency.agentName}</h4>
                  <p style="color: var(--meadow-lt); font-size: 0.88rem; margin: 3px 0; font-weight: 700;">${prop.agency.name}</p>
              ...
              <div style="background: rgba(255,255,255,0.08); padding: 1rem; border-radius: 10px; font-size: 0.88rem; margin-bottom: 1.25rem; display: flex; flex-direction: column; gap: 6px;">
              ...
              <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                <a href="https://wa.me/${prop.agency.whatsapp}?text=Hi%20${encodeURIComponent(prop.agency.agentName)},%20I%20am%20interested%20in%20listing%20ID%20${prop.id}%20(${encodeURIComponent(prop.title)})" 
                   ...
                   style="width: 100%; padding: 0.85rem; font-size: 0.95rem; font-weight: 800; border-radius: 10px; display: flex; align-items: center; justify-content: center; gap: 8px; text-decoration: none;">
            ...
            <div style="background: var(--paper); border-radius: 16px; padding: 1.5rem; border: 2px solid var(--border-dk); box-shadow: var(--shadow-md);">
              <h4 style="color: var(--forest-dk); font-size: 1.1rem; font-weight: 800; margin-bottom: 0.5rem; display: flex; align-items: center; gap: 6px;">
              ...
              <div style="background: var(--cream); padding: 1rem; border-radius: 10px; text-align: center; border: 1px solid var(--border-dk); margin-bottom: 1rem;">
                <div style="font-size: 0.75rem; color: var(--forest); font-weight: 700; font-family: var(--font-mono);">ESTIMATED MONTHLY E-EMI</div>
                <div style="font-size: 1.5rem; font-weight: 800; color: var(--rani-dk); font-family: var(--font-mono);">
